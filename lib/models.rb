...
def ruby_syntax(content)
    h = Hpricot(content.strip)
...
module Nesta
  class FileModel
    ...
    def to_html
      case @format
      when :mdown
        ruby_syntax Maruku.new(markup).to_html
      when :haml
        ruby_syntax Haml::Engine.new(markup).to_html
      when :textile
        ruby_syntax RedCloth.new(markup).to_html
      end
    end
